...
val_size = 6912
dt = 96
train_x = logRet[:val_size].reshape(int(0.375*dt), dt, 2)
val_x = logRet[val_size:train_size].reshape(int(0.125*dt), dt, 2)
full_train_x = logRet[:train_size].reshape((dt//2),dt,2)
test_x = logRet[train_size:(n-dt)].reshape(int(0.25*dt),dt,)
train_y = logRet[dt:(val_size+dt)].reshape(int(0.75*dt), dt, 1)
val_y = logRet[(val_size+dt):(train_size+dt)].reshape(int(0.25*dt), dt, 1)
full_train_y = logRet[dt:(train_size+dt)].reshape(dt,dt,1)
test_y = logRet[(train_size+dt):n].reshape(int(0.5*dt),dt,1)
